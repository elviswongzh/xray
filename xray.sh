...
getVersion() {
	rm -rf /tmp/xray
	mkdir /tmp/xray
	wget -O /tmp/xray/latest https://api.github.com/repos/XTLS/Xray-core/releases/latest
	RETVAL=$?
	# 输出xray version命令的结果，取第一行，并截取以空格为单位的第二列
	CUR_VER=$(xray version | awk 'NR==1{print}' | awk '{print $2}')
	
	# 输出tmp/xray/latest文件，过滤出tag_name，并截取以:为单位的第二列，并去除字母v标点符号"标点符号,
	NEW_VER=$(cat /tmp/xray/latest | grep tag_name | awk -F: '{print $2}' | awk '{gsub(/v|"|,/,"",$1); print $1}')
...
installXray() {
	DOWNLOAD_LINK="${V6_PROXY}https://github.com/XTLS/Xray-core/releases/download/v${NEW_VER}/Xray-linux-$(archAffix).zip"
...
outputVlessWS() {
	raw="$uid@$IP:$port?encryption=none&security=tls&type=$network&host=$domain&path=$wspath"
	link="vless://${raw}"
...
menu() {
	...
	echo -e "# ${GREEN}作者${PLAIN}: 黄洪达                                              #"
	echo -e "# ${GREEN}邮箱${PLAIN}: java_dotnet@126.com                                 #"                    #"
	echo "#############################################################"
